...
def get_rotation_matrices(dim, end_x, end_y, theta=10000.0, device=None, dtype=None):
    ...
    x_angles = torch.ger(x_mults, base_angles)
    y_angles = torch.ger(y_mults, base_angles)
    
    sterm_x, cterm_x = torch.sin(-x_angles), torch.cos(-x_angles)
    sterm_y, cterm_y = torch.sin(-y_angles), torch.cos(-y_angles)
    
    rotmat_x = torch.stack([torch.stack([cterm_x, -sterm_x], dim=-1), torch.stack([sterm_x, cterm_x], dim=-1)], dim=-1)
    rotmat_y = torch.stack([torch.stack([cterm_y, -sterm_y], dim=-1), torch.stack([sterm_y, cterm_y], dim=-1)], dim=-1)
    
    rotmats = torch.cat([rotmat_x, rotmat_y], dim=1).unsqueeze(0).unsqueeze(0)
